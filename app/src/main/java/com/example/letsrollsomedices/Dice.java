...
/*
Я не ебу как правильно комменты делать
Класс, который позволяет считывать строку с кубиками и выдавать результат, или чёт типа того
Сделал Правой Илья
Как пользоваться: вызываете метод doTheDiceRoll, класса Dice, туда передаёте строку типа:
знак число "d" число - это если кубик
знак число - это если модификатор
ВСЕГДА, БЛЯТЬ, ТЫ ПОНЯЛ МЕНЯ, ВСЕГДА ПИШИ ЗНАК, ДАЖЕ ЕСЛИ МОДИФИКТОР/БРОСОК ПОЛОЖИТЕЛЬНЫЙ, ИНАЧЕ Я НАЙДУ ТЕБЯ
Пример:
"-40+12d6-2" или "+4d12+2-3-6d6+12+2d8-2"
На вопрос, а херали здесь нет проверки, вопрос простой: Это для другой проги + я дебил и лентяй
*/
public class Dice {
    private final ArrayList<String> dices= new ArrayList<String>();
    private final ArrayList<String> modifications =new ArrayList<String>();
    ...
    public void doTheDiceRoll(String input){
        ...
        string=matcher.replaceAll("");//После нахождения всех кубов, удаляем их нахуй, так как я хз как сделать иначе
    ...
    private void addModificationsToResult(){
        ...
        addDicesToResult();//Следующий, пожалуйста
    }
